...
$Questions = array($S9,$S10,$S11,$S12,$S13);
...
foreach($Questions as $sec){
    ...
    foreach($sec as $q){
        ...
        if ($qNum <= 0){
            ...
            $cap_url = "createCAP.php?inspection_id={$_SESSION['formID']}&sec_id={$secNum}&question_id={$qNum}";
            echo "<input type = 'button' value='Add Action' onclick= window.open('{$cap_url}','CAP','width=400,height=400')>";
