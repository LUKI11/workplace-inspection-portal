...
foreach($Questions as $sec){
    ...
    foreach($sec as $q){
        ...
        if ($qNum <= 0){
            ...
            $q_id = $result[$qNum-1]['inspection_id'];
            ...
            //printf("Select returned %d rows.\n", mysqli_num_rows($note));
            
            //$note = $note->fetch_object();
            //$comment =  $note->note;
            ...
            echo "<p>{$q}</p>";
            echo "<p>Selected: {$row['inspection_selection']}</p>";      
            echo "<br>
            <textarea cols='60' rows='1' name='{$secNum}:{$qNum}:n' placeholder='Additional comment'>{$comment}</textarea>
            <br>";
            echo "</div>";  
            */
