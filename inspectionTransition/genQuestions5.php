...
<div> 
    Go to: 
    <select class='selectpicker' name='section_tp' onchange='location = this.value;' value='This section'>
    <option selected disabled>Select section</option>
     <option value='inspection_form1.php'>Section: 1 - 5</option>
     <option value='inspection_form2.php'>Section: 6 - 9</option>
     <option value='inspection_form3.php'>Section: 10 - 13</option>
     <option value='inspection_form4.php'>Section: 14 - 15</option>
     <option value='inspection_form5.php'>Section: 16 - 18</option>
    </select>
</div>
";
...
foreach($Questions as $sec){
    ...
    $sql = "SELECT * FROM report_questions, report WHERE report.inspection_id  = ".$id.
    ...
    foreach($sec as $q){
        ...
        if ($qNum <= 0){
		   ...
		   echo $note->note;
			...
			//method for users to create cap
            $cap_url = "createCAP.php?inspection_id={$id}&sec_id={$secNum}&question_id={$qNum}";
            echo "<br><input type = 'button' class='btn btn-primary' value='Add Corrective Action' onclick= window.open('{$cap_url}','CAP','width=400,height=400')>";
			 echo"<br>";

            //method to upload images
            $oldpic = $result['inspection_images'];
            echo "   
				<img src='../uploads/$oldpic'  height='40' />
				<input name='{$secNum}:{$qNum}:pic' type='file'> ";
            ...
            echo "<br><textarea class='form-control' name='{$secNum}:{$qNum}:n' placeholder='Additional comment'>{$comment}</textarea>";
            echo "</div><br>";
